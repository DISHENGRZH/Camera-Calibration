...
def write_to_json(input_address, newcameramtx, distcoef):
    # 获取输入地址的后三位作为名称
    name = input_address[-11:]

    # 构造数据结构
    data = {
        "name": name,
        "distCoef": distcoef.tolist(),
        "K": newcameramtx.tolist()  # 将numpy数组转换为列表
    }

    # 写入到JSON文件中
    camera_path = os.path.join(input_address, os.pardir)
    output_file = os.path.join(camera_path, f"{name}.json")
    with open(output_file, "w") as f:
        json.dump(data, f, indent=4)

# 图片在当前文件夹的位置
parent_folder = r"\\SEUVCL-DATA-02\BodyStudio\Cap"
sub_folder = [f.path for f in os.scandir(parent_folder) if f.is_dir()]
for file_in in sub_folder:
    ...
    write_to_json(file_in, newcameramtx, dist)